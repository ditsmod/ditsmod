...
`Logger` та `LoggerConfig` класом `BunyanService` та інстансом класа `loggerConfig` відповідно.
Через це, контролер у `SomeModule` буде використовувати саме `BunyanService`, коли він звертається
до логера:
...
Але у `PinoModule` та `WinstonModule` зроблено підміни by default логера, причому зроблено це в
