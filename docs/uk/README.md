...
сказати, надаєте інший **провайдер** для створення інстансу класу `FirstService`.
...
До речі, якщо `canActivate()` повертає `number`, то Ditsmod інтерпретує це як номер статусу (403,
401 і т.п.), який треба повернути у відповіді на HTTP-запит.
...
Але майте на увазі, що ви отримуєте інжектор, що створив інстанс цього провайдера.
Це означає, що якщо інстанс створений на рівні застосунку - ви отримаєте інжектор на
рівні застосунку. Якщо оголосили його на рівні модуля - отримаєте інжектор для конкретного модуля.
Якщо оголосили на рівні запиту - отримаєте інжектор для даного запиту.
...
`Module2` та `Module3`. Ви очікували, що `Service3` буде мати поведінку таку, як вона
задокументована у `Module3`, але насправді спрацювала та версія, що експортується із `Module2`.
