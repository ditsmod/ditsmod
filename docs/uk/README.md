...
Коли ми передаємо провайдер у один із масивів: `providersPerApp`, `providersPerMod` чи `providersPerReq` - тим самим ми декларуємо
на якому рівні буде створюватись [одинак][12] даного провайдера. Але це не теж саме, що **область видимості** провайдерів.

Наприклад, якщо у `SomeModule` ми оголосили `ConfigService` на рівні `providersPerMod`, це означає, що одинак `ConfigService` буде
...
Разом із тим, щоб збільшити **область видимості** `ConfigService` ми маємо його експортувати із `SomeModule`,
після чого усі модулі, що імпортують `SomeModule`, теж матимуть свій окремий одинак `ConfigService` на рівні модуля.

Як бачите, **область видимості** провайдерів розширюється за допомогою експорта цих провайдерів з подальшим імпортом модулів, де вони оголошені.
Хоча, якщо провайдери оголошені у кореневому модулі, і вони потрібні вам в іншому модулі, імпортувати кореневий модуль не потрібно.
Достатньо в кореневому модулі зробити експорт потрібних провайдерів, після чого їх область видимості збільшиться на увесь застосунок.
