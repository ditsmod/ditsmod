...
- роут
- сервіс
- DI (тобто **D**ependency **I**njection)
- провайдер сервіса
...
як мінімум, доступ до об'єкта відповіді (Response). В наступному прикладі створено два маршрути,
...
але взагалі без модифікатора, `res` вже буде простим параметром і буде доступним лише в конструкторі.

Щойно в конструкторі ми отримали інстанс класу `Response`, а він якраз і представляє собою сервіс,
що згадувався раніше як складова архітектури Ditsmod.
