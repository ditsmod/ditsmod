...
- [Запуск](#запуск)
- [Загальна картина архітектури застосунку Ditsmod][145]
...
## Загальна картина архітектури застосунку Ditsmod

В наступних розділах будуть докладно розібрані окремі складові застосунку Ditsmod, а зараз давайте
глянемо яку загальну картину архітектури він має.

Одним із найбільших блоків архітектури застосунку Ditsmod - є модуль. Коротко кажучи, модуль
об'єднує у собі набір класів, призначених для чітко визначеної мети. Добре оформлений
модуль не повинен робити "усе на світі".

Наприклад, модуль системи безпеки має чітко визначену мету - безпека доступу до застосунку,
та безпека управління застосунком. Тут не повинні оголошуватись класи, наприклад, що перекладають
повідомлення різними мовами, що відправляють пошту, що пишуть логи і т.п.

Також "чітко визначеною метою" може вважатись обробка розділу, прив'язаного до певного URL.
Наприклад, один модуль може обробляти усі HTTP-запити за адресою `/api/users`, інший
модуль - за адресою `/api/posts`.

При оголошенні кожного модуля прописується: які саме зовнішні модулі будуть використовуватись в
межах поточного модуля, та які саме класи із поточного модуля будуть доступними для використання
у зовнішніх модулях. Уся ця інформація передається в інший великий блок архітектури
Ditsmod - у систему [Dependency Injection][8], скорочено - DI.

На етапі ініціалізації застосунку, DI збирає таку інформацію з усіх модулів Ditsmod, після чого
аналізує **залежність** кожного із класів від інших класів та готує відповідні набори щоб
**вирішити цю залежність**.
...
На практиці це означає, наприклад, що хоча Ditsmod вже використовує у своїй роботі by default
логер для запису логів у консоль, але ви можете підмінити його своїм логером для запису логів,
наприклад, у файл.
...
Це ж саме стосується будь-яких інших класів, від яких залежать інші класи - їх усіх можна
підміняти через DI. Тобто окрім логера, ви можете підміняти роутер, парсер тіла запиту, обробник
помилок, різні by default конфігурації і навіть самі класи запитів та відповідей.
...
[Dependency Injection][8] (скорочено - DI).
...
Щоб надавати в конструкторі класу те, що ви запитуєте, [DI][8] повинен бути проінструктований
