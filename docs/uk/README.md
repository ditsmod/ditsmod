...
3. Ще нижче в ієрархії йде інжектор на рівні роуту, він бачить усі провайдери, що ви
передаєте у `providersPerRou` та `providersPerMod` конкретного модуля, а також у
`providersPerApp` будь-де в застосунку. Кількість цих інжекторів дорівнює кількості визначених
роутів в контролерах усього застосунку.
4. Самим нижчим в ієрархії є інжектор на рівні HTTP-запиту, він бачить усі провайдери, що ви
передаєте у `providersPerReq`, `providersPerRou` та `providersPerMod` конкретного модуля, а також у
...
час, `Request` та `Response` оголошені на рівні запиту, і саме тому їх не бачить вищий в
ієрархії інжектор.
