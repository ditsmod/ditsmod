...
describe('DefaultHttpErrorHandler', () => {
  ...
  it('default error with some message', () => {
    ...
    (err as any).status = HttpStatus.PAYLOAD_TO_LARGE;
    expect(() => errorHandler.handleError(err, ctx)).not.toThrow();
    expect(rawRes.statusCode).toBe(HttpStatus.PAYLOAD_TO_LARGE);
  ...
  it('custom error with msg1', () => {
    ...
    expect(() => errorHandler.handleError(err, ctx)).not.toThrow();
    expect(rawRes.statusCode).toBe(HttpStatus.INTERNAL_SERVER_ERROR);
    expect(rawRes.end).toHaveBeenCalledWith(JSON.stringify({ error: 'one' }));
    expect(rawRes.end).toHaveBeenCalledTimes(1);
  ...
  it('custom error with status and level changed', () => {
    const msg1 = 'one';
    const err = new CustomError({ msg1, status: HttpStatus.CONFLICT, level: 'fatal' });
    expect(() => errorHandler.handleError(err, ctx)).not.toThrow();
    expect(rawRes.statusCode).toBe(HttpStatus.CONFLICT);
    expect(rawRes.end).toHaveBeenCalledWith(JSON.stringify({ error: 'one' }));
    expect(rawRes.end).toHaveBeenCalledTimes(1);
