...
const words = fs.readFileSync(__dirname + '/long-running.js', 'utf8').split(/\s+/);
const levels = ['trace', 'debug', 'info', 'warn', 'error', 'fatal'];
let timeout;

const log = new Logger({ name: 'lr', level: 'debug' });
...
function logOne() {
  const level = randchoice(levels);
  const msg = [randchoice(words), randchoice(words)].join(' ');
  const delay = randint(300);
  // console.warn('long-running about to log.%s(..., "%s")', level, msg)
  log[level]({ word: randchoice(words), delay }, msg);
