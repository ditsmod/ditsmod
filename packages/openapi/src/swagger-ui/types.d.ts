declare module 'swagger-ui' {
  interface SwaggerUIOptionsBase {
    ...
    configUrl?: string;
    /**
     *  Required if `domNode` is not provided. The ID of a DOM element inside which
     * `SwaggerUI` will put its user interface.
     */
    dom_id?: string;
    /**
     * Required if `dom_id` is not provided. The HTML DOM element inside which `SwaggerUI`
     * will put its user interface. Overrides `dom_id`.
     */
    domNode?: Element;
    /**
     * A JavaScript object describing the OpenAPI definition. When used, the `url` parameter will
    ...
    url?: string;
    /**
     * An array of API definition objects
     * (`[{url: "<url1>", name: "<name1>"},{url: "<url2>", name: "<name2>"}]`) used by Topbar
     * plugin. When used and Topbar plugin is enabled, the `url` parameter will not be parsed.
     * Names and URLs must be unique among all items in this array, since they're used
     * as identifiers.
     */
