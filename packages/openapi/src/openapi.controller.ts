...
import { controller, RequestContext } from '@holu/rest';
...
export class OpenapiController {
  ...
  async getIndex() {
    ...
    this.ctx.setContentType('text/html; charset=utf-8').send(indexHtml);
  }
  ...
  async getYaml(@ctx(OAS_CONFIG_FILES) configFiles: OasConfigFiles) {
    this.ctx.setContentType('text/yaml; charset=utf-8').send(configFiles.yaml);
  }
  ...
  async getJson(@ctx(OAS_CONFIG_FILES) configFiles: OasConfigFiles) {
    this.ctx.setContentType('application/json; charset=utf-8').send(configFiles.json);
  }
  ...
  getJavaScript() {
    this.ctx.setContentType('text/javascript; charset=utf-8');
    return new Promise<void>((resolve, reject) => {
      createReadStream(`${webpackDist}/openapi.bundle.js`)
        .on('close', resolve)
        .on('error', reject)
        .pipe(this.ctx.rawRes);
    });
  }
}
