...
It is important to understand that `injectable` decorators are only needed because there is no way in JavaScript code to specify the type of the parameter in the constructor, as it is done in TypeScript code. The role of `injectable` decorators is quite simple - they allow you to transfer to JavaScript code those metadata that are in TypeScript code in class constructors. For example, the work of the decorator `injectable` for the class `Service2` will be that `Service1` will be remembered as the first parameter in the constructor. This metadata is set in static properties of classes, using unique symbols generated by methods of the standard class `Symbol`.

Thanks to the work of DI, we can request an instance of `Service3` anywhere in our application:
