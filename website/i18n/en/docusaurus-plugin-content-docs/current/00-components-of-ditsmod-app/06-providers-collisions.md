...
3. providers with token `Service1` are not identical in `Module1` and `Module2`, i.e. from module `Module2` can be exported, for example, object `{ token: Service1, useValue: {} }`, and from `Module1``Service1` can be exported as a class.
