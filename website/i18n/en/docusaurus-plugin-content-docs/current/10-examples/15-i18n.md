...
i18n is an abbreviation of the internationalization. The @ditsmod/i18n module provides basic functionality for translating system messages (issued by the [Ditsmod][1] application at runtime) and provides the ability to easily extend dictionaries. In fact, you use ordinary services as dictionaries for translation, so the text for translation can be taken both from TypeScript files and from databases. The work of @ditsmod/i18n is designed so that each current module can have its own translation, and that the translation of any imported module can be changed or supplemented.

You can view the code with examples of the use of `@ditsmod/i18n` in the [Ditsmod repository][2], although it is more convenient to view it locally, so it is better to clone it first:
...
Each basic dictionary must implement the `Dictionary` interface, which has a single requirement - that the dictionary has a `getLng()` method that returns the abbreviation of the language name according to the standard [ISO 639][3] (for example, abbreviations for English and Ukrainian languages ​​- en, uk).
