...
But in most cases, the language is selected through an HTTP request. By default, `DictService` takes the locale from the `lng` parameter in `this.ctx.req.queryParams`, but you can change the name of this parameter by passing the `lngParam` option:
...
Although the default value of the query language is determined via `this.ctx.req.queryParams`, you can easily change the logic of determining the query language, for example, by `accept-language` headers. To do this, it is enough to change the `dictService.lng` getter.
