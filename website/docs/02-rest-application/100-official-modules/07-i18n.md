...
i18n - це скорочення від слова internationalization. Модуль `@holu/i18n` забезпечує базову функціональність для перекладу системних повідомлень (що видає Holu-застосунок під час роботи), і надає можливість для легкого розширення словників. Фактично ви використовуєте звичайні сервіси у якості словників для перекладу, тому текст для перекладу можна брати як з TypeScript-файлів, так і з баз даних. Робота `@holu/i18n` спроектована таким чином, щоб кожен поточний модуль міг мати власний переклад, і щоб можна було змінити чи доповнити переклад будь-якого імпортованого модуля.

Проглянути код з прикладами використання `@holu/i18n` можна у [репозиторію Holu][1], хоча зручніше його проглядати локально, тому краще спочатку клонувати його:
...
Вище показано як клонувати готовий приклад з `@holu/i18n`, а коли вам потрібно встановити даний модуль у ваш застосунок, це можна зробити так:
...
```ts
import { DictGroup, getDictGroup } from '@holu/i18n';
...
import { I18nModule, I18nOptions, I18N_TRANSLATIONS, Translations } from '@holu/i18n';
...
import { I18N_TRANSLATIONS, I18nModule, I18nProviders } from '@holu/i18n';
...
[1]: https://github.com/holujs/holu/tree/main/examples/15-i18n
