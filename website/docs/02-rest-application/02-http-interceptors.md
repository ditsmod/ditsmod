...
Інтерсептори можна централізовано підключати або відключати, не змінюючи при цьому код методів контролерів, до яких вони прив'язуються. Як і контролери, інтерсептори можуть працювати [в режимі request-scoped чи route-scoped][109]. На відміну від route-scoped, в режимі request-scoped вони мають доступ до інжектора на рівні запиту, тому вони можуть викликати сервіси на рівні запиту, роуту, модуля чи застосунку. З іншого боку, в режимі route-scoped їхні інстанси створюються на рівні роуту, відповідно - для них доступні сервіси на рівні роуту, модуля чи застосунку.

## Схема обробки HTTP-запиту {#http-request-processing-scheme}

### Робота в режимі request-scoped {#request-scoped-mode}
...
1. `RequestDispatcher`;
2. `HttpFrontend`;
3. `RequestScopedGuardedInterceptor`;
4. інші інтерсептори;
5. `HttpBackend`, який запускає метод контролера.

Оскільки ланцюжок промісів починається від `RequestDispatcher`, і закінчується методом контролера, то резолвиться цей ланцюжок у зворотньому порядку - від методу контролера до `RequestDispatcher`. Це означає, що в інтерсепторі ви можете слухати результат резолву проміса, що повертає метод контролера.

Окрім цього, оскільки інстанси `RequestDispatcher`, `HttpFrontend`, `RequestScopedGuardedInterceptor` та `HttpBackend` створюються за допомогою DI, ви можете їх підміняти своєю версією відповідних класів. Наприклад, якщо ви хочете не просто відправити 404-ий статус у випадку відсутності потрібного роута, а хочете ще й додати певний текст чи змінити заголовки, ви можете підмінити [RequestDispatcher][7] своїм класом.

### Робота в режимі route-scoped {#route-scoped-mode}
...
[1]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/rest/src/interceptors/tokens-and-types.ts#L14-L16
[2]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/rest/src/interceptors/default-http-frontend.ts
[3]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/rest/src/interceptors/default-http-backend.ts
[5]: https://expressjs.com/en/guide/writing-middleware.html
[7]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/rest/src/services/request-dispatcher.ts
[8]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/rest/src/types/route-data.ts
[9]: https://github.com/holujs/holu/blob/3.0.0-next.15/packages/body-parser/src/body-parser.extension.ts#L54
