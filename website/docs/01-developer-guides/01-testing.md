...
Перш за все, зверніть увагу у першому рядку що при імпорті бібліотеки `supertest` одночасно використовується ключові слова `import` та `reuire`. Ця особливість виникає через те, що `supertest` по дефолту експортує функцію, а не об'єкт, і це протирічіть стандартам експорту ES2015+.
