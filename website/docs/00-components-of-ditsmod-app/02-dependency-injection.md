...
Давайте спробуємо розібратись, як саме DI отримує повну інформацію про параметри конструктора класу, для успішного вирішення вказаної залежності. В наступному прикладі показано клас, в конструкторі якого є три параметра, кожен із яких має свій TypeScript-тип:
...
Тобто, TypeScript-типи зникають з параметрів конструктора у JavaScript-коді. Отже, під час компіляції TypeScript повинен згенерувати спеціальну функцію, яка дозволить зберегти TypeScript-типи, які ми передаємо у параметри конструктора. Це функція може бути, наприклад, такою:
...
Приблизно так воно насправді і працює. Подібні функції TypeScript-компілятор генерує у кожному файлі, де є класи з декоратором `injectable`. Очевидно, що в нашому прикладі функція `setConstructorTokens` може працювати тільки з JavaScript-значеннями, вона не зможе прийняти в якості аргументів такі типи, які у TypeScript-коді ми оголошуємо з ключовими словами `interface`, `type`, `enum` і т.п., бо їх не існує у JavaScript-коді.

JavaScript-значення, які приймає `setConstructorTokens`, у контексті DI називають **токенами**. Передати токен можна у короткій або довгій формі вказання залежності. Давайте знову розглянемо попередній приклад:
...
Токеном може бути референс на клас, об'єкт чи функцію, також у якості токену можна використовувати примітивні значення, окрім `undefined`. Для довгої форми вказання залежностей, рекомендуємо використовувати інстанс класу `InjectionToken<T>` у якості токена:
...
Як можна здогадатись, при вирішенні залежності, DI братиме токени з параметрів конструктора певного класа, і шукатиме для них значення у реєстрі DI. Якщо усі необхідні токени знайшлись у реєстрі, значить їхні значення передаються у конструктор, і таким чином успішно вирішується залежність даного класа.
...
- `useValue` - сюди передається будь-яке значення, окрім `undefined`, DI його видаватиме без змін. Приклад такого провайдера:
  ...
  ```
- `useFactory` - сюди передається [tuple][11], де на першому місці повинен бути клас, а на другому місці - метод цього класу, який повинен повернути якесь значення для вказаного токена. Наприклад, якщо клас буде таким:
...
Ditsmod під капотом використовує метод `injector.get()`, коли DI вирішує залежність, яку знаходить в конструкторі класу. 
