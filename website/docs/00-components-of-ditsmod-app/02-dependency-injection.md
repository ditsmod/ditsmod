...
У DI є реєстр, що містить мапінг між токеном та значенням для певної залежності. У попередньому прикладі у конструкторі - `FirstService` - це токен залежності, і DI буде вирішувати цю залежність шукаючи у своєму реєстрі відповідне значення саме по даному токену. 
...
Як бачите, декоратор `Inject` приймає токен для певної залежності.
...
Приклади використання цих об'єктів показані у розділі [Підміна провайдерів][100].
