...
Одна з головних особливостей DI - це вирішення певної залежності, а при повторному запиті цієї залежності - видача готового (вирішеного) зачення з кешу. Тобто DI має реєстр, що містить мапінг між певною залежністю і відповідним значенням для неї. Щоб однозначно ідентифікувати кожну залежність, використовуються так звані **токени**, що можуть мати будь-який JavaScript-тип, окрім `undefined`. Реєстр залежностей формується при написанні Ditsmod-застосунку, і схематично він виглядає наступним чином:
...
Для вирішення залежностей, DI буде шукати у своєму реєстрі відповідні значення саме по токенам. В конструкторах модулів, сервісів чи контролерів вказуються саме токени.
...
Декоратор `inject` дозволяє використовувати альтернативний токен, він необхідний для отримання в конструкторі масивів, enum, чи будь-якого іншого значення:
...
Використовуючи ці масиви, Ditsmod формує з них чотири різні інжектори, що пов'язані між собою ієрархічним зв'язком. Самий вищий в ієрархії - інжектор на рівні застосунку, його реєстр формується з масиву `providersPerApp`. Другий в ієрархії - інжектор на рівні модуля, третій - інжектор на рівні роуту, ну і четвертий - інжектор на рівні HTTP-запиту. Коли ви плануєте передавати провайдери до перерахованих вище масивів, обов'язково враховуйте ієрархію інжекторів. Нагадаємо, що вищі в ієрархії інжектори не мають доступу до нижчих в ієрархії інжекторів.
