...
Указані тут значення DI створює використовуючи **провайдери**. Отже, щоб DI міг вирішити певну залежність, спочатку необхідно передати відповідний провайдер до реєстру DI, а потім DI буде видавати значення цього провайдера по його токену. Іншими словами, провайдери фактично вирішують залежність. Тому якщо ви вказали певну залежність у класі, але не передали відповідного провайдера, DI не зможе вирішити дану залежність. Про те, як саме можна передавати провайдери до DI, йдеться [в наступному розділі][100]. Провайдери мають такий тип:
...
- `useClass` - передається клас, DI буде робити інстанс цього класу. Приклад такого провайдера:
  ...
  ```
- `useValue` - передається будь-яке значення, окрім `undefined`, DI його видаватиме без змін. Приклад такого провайдера:
