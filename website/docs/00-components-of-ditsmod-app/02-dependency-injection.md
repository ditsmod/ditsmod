...
Під капотом, Ditsmod використовує [@ts-stack/di][9] v2 у якості бібліотеки для Dependency Injection (скорочено - DI), вона має наступні базові поняття:
...
В системі DI, залежність - це зокрема те, що ви хочете отримати в кінцевому результаті в конструкторах контролерів, сервісів, модулів. Наприклад, якщо в конструкторі сервісу ви прописуєте ось таке:
...
У DI є реєстр, що містить мапінг між токеном та його значенням. Цей реєстр формує сам користувач DI, і схематично він виглядає наступним чином:
...
Для вирішення залежностей, DI буде шукати у своєму реєстрі відповідні значення саме по токенам. По-суті, токен - це ідентифікатор для певної залежності. В конструкторах модулів, сервісів чи контролерів вказуються саме токени. 

В розділі [Передача провайдерів в реєстр DI][100] ви дізнаєтесь, що DI дозволяє для одного і того ж токена передавати будь-яке значення. Цю особливість зручно використовувати для unit-тестування, оскільки замість справжньої залежності, до реєстра можна передати mock чи stub. В такому разі реєстр DI вигладає приблизно наступним чином:
...
Токен може мати будь-який тип, але на даний момент у DI є особливість, через яку DI не бачить різниці між різними типами _масиву_ чи _enum_. Окрім цього, ви повинні пам'ятати, що токен повинен залишитись у JavaScript-файлі після компіляції з TypeScript-коду, тому у якості токена не можна використовувати інтерфейси або типи, що оголошені за допомогою ключового слова `type`. Декоратор `inject` дозволяє використовувати альтернативний токен, він необхідний для отримання в конструкторі масивів, enum, чи будь-якого іншого значення:
...
Коли використовується `inject`, DI використовує переданий в нього токен, а тип змінної, перед якою знаходиться цей декоратор, ігнорує, тому цей тип може бути навіть інтерфейсом.
...
Вище було сказано, що у DI є реєстр, що містить мапінг між токеном та значенням, яке потрібно видавати для певної залежності.
...
Так ось якраз ці значення DI створює використовуючи провайдери. По-суті, DI вирішує залежність за допомогою відповідних провайдерів. Отже, щоб вирішити певну залежність, спочатку необхідно передати відповідний провайдер до реєстру DI, а потім DI буде видавати інстанс цього провайдера по його токену. Провайдери можуть бути або класами, або об'єктами з таким типом:
...
Використовуючи дані масиви Ditsmod формує з них чотири різні інжектори, що пов'язані між собою ієрархічним зв'язком. Самий вищий в ієрархії - інжектор на рівні застосунку, його реєстр формується з масиву `providersPerApp`. Другий в ієрархії - інжектор на рівні модуля, третій - інжектор на рівні роуту, ну і четвертий - інжектор на рівні HTTP-запиту. Коли ви плануєте передавати провайдери до інжекторів, обов'язково враховуйте їхню ієрархію. Нагадаємо, що вищі в ієрархії інжектори не мають доступу до нижчих в ієрархії інжекторів.

Наприклад, якщо ви напишете клас, що має залежність від HTTP-запиту, ви зможете його передати тільки у масив `providersPerReq`, бо тільки з цього масиву формується інжектор, який буде мати доступ до HTTP-запиту. З іншого боку, цей провайдер матиме доступ до усіх своїх батьківських інжекторів: на рівні роуту, модуля, та застосунку. При цьому варто пам'ятати, що якщо провайдерами є класи чи фабрики (об'єкти з властивістю `useFactory`), їхні інстанси будуть створюватись кожен раз, за кожним HTTP запитом.

Також ви можете написати клас і передати його в масив `providersPerMod`, в такому разі він може залежати тільки від провайдерів на рівні модуля, або на рівні застосунку. Якщо ви спробуєте додати в його конструктор провайдери, які ви передали в масив `providersPerRou` чи `providersPerReq`, ви отримаєте помилку про те, що ті провайдери не знайдені. Інстанси провайдерів на рівні модуля створюються єдиний раз після запуску застосунку.

Звичайно ж, ви також можете передавати провайдери на рівень застосунку чи роуту, і вони матимуть аналогічні обмеження з врахуванням ієрархії інжекторів. 
