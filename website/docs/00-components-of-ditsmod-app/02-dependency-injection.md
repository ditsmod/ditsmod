...
Якщо провайдер представлено у вигляді об'єкта, у його властивості можуть передаватись наступні типи:
...
Інструкція `useClass` відрізняється від інструкції `useValue` тим, що для `useClass` інжектор буде намагатись створити інстанс класу, а для `useValue` буде видавати незмінене значення.
