...
В системі DI, залежність - це усе те, що ви запитуєте у конструкторах контролерів, сервісів, модулів. Наприклад, якщо в конструкторі сервісу ви прописуєте ось таке:
...
1. спочатку буде створено інстанс `FirstService`;
2. для створення інстансу `SecondService`, в його конструктор буде передано інстанс `FirstService`.

## Токен залежності

Токен залежності - це ідентифікатор для певної залежності. Тобто "залежність" - це те, що потрібно отримати в кінцевому результаті в конструкторі, а "токен" - це ідентифікатор, за допомогою якого ця залежність буде шукатись в DI. У попередньому прикладі у конструкторі - `FirstService` - це токен залежності.
...
DI вирішує залежність за допомогою відповідних провайдерів. На одну залежність, в DI потрібно передавати один або декілька провайдерів у метадані модуля чи контролера, хоча інколи вони передаються напряму в інжектори (див. наступний розділ).

У `@ts-stack/di` провайдер може бути або класом, або об'єктом з таким типом:
...
1. ви не передали потрібний провайдер до DI;
