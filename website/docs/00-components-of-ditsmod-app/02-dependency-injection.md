...
В конструкторах класів залежності указуються за допомогою так званих **токенів**. По-суті токен - це ідентифікатор, з яким асоціюється певна залежність, причому ця асоціація відбувається вже у Node.js runtime. Тобто типи даних, які є у TypeScript і відсутні у JavaScript, не підходять для використання їх у якості токенів.

Токен може мати будь-який JavaScript-тип, але на даний момент у DI є обмеження, через яке DI не бачить різниці між різними примітивними типами, різними типами _масивів_ чи _enum_. Окрім цього, ви повинні пам'ятати, що токен повинен залишитись у JavaScript-файлі після компіляції з TypeScript-коду, тому у якості токена зокрема не можна використовувати інтерфейси або типи, що оголошені за допомогою ключового слова `type`.
...
В даному разі `FirstService` використовується одночасно і як тип змінної, і в якості токену для указання залежності `SecondService` від інстансу класу `FirstService`. Це означає, що `FirstService` повинен бути класом, але не інтерфейсом чи примітивним типом.
