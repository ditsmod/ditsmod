...
Інколи останній пункт (коли інстанс `Service3` повертається з кешу інжектора), є небажаним. В такому разі ви можете скористатись методом `injector.instantiate()`, який дуже схожий на роботу `injector.get()`, але він кожен раз створює новий інстанс даного провайдера, причому робитиме це в контексті поточного інжектора.
