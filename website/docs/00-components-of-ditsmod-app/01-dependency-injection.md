...
Як говорить нам помилка, мова йде про якісь **провайдери**. Але що це таке, і чому кидається помилка? Як мінімум, нам вже зрозуміло, що інжектор вимагає передати йому не залежності `Service3`, які рефлектор може самостійно прочитати, а провайдери для цього класу. Щоб краще зрозуміти що собою являють провайдери та чому їх вимагає інжектор, давайте перепишемо попередній приклад передавши інжектору провайдери в іншій формі:
...
Тепер, коли ми передали до інжектора провайдери у вигляді інструкцій, стає більш зрозуміло, що інжектору необхідні інструкції для мапінгу між тим, що у нього запитують (токеном), та тим що він видає (значенням). В документації такий мапінг також може називатись **реєстром провайдерів**. Що стосується токенів, то для інжектора токен - це ідентифікатор для пошуку значення у реєстрі провайдерів. Окрім цього, тепер можна здогадатись, що під час сканування залежностей класу [рефлектор][108] повертає саме токени, а не провайдери, тому інжектор не може задовільнитись використанням лише рефлектора, без передачі провайдерів. Тому під час створення інжектора, йому необхідно передавати провайдери з тими токенами, які у нього будуть запитуватись зокрема через `injector.get()`.

Треба пам'ятати, що токени працюють в JavaScript-коді, а не у TypeScript-коді, тому у якості токенів не можуть виступати сутності, які оголошені з ключовими словами `interface`, `type`, `declare`, `enum` і т.д., оскільки після компіляції їх не буде у JavaScript-коді. Окрім цього, токени не можна імпортувати з ключовим словом `type`, оскільки у JavaScript-коді такого імпорту не буде.

Токени провайдерів можуть мати наступні типи даних:

1. Будь-який об'єкт, окрім `null` та масиву. Рекомендуємо використовувати інстанс класу `InjectionToken<T>` у якості токена, оскільки він є дженеріком, який приймає параметр типу `T`. Цей параметр можна використовувати щоб асоціювати даний токен зі значенням, яке буде повертатись з реєстру провайдерів.
2. Також добре працюють токени з наступними типами: `string`, `number` чи `symbol`.
