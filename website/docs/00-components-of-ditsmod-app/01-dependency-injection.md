...
Інжектор - це основний механізм, який реалізує патерн Dependency Injection в Ditsmod. Кінцева мета інжектора - видавати значення по певному ідентифікатору, який називається **токеном**. Інжектор має такі компоненти:

1. **Зберігання провайдерів**: Інжектор містить реєстр провайдерів, які по-суті є інструкціями про те, як створити значення для токенів.
2. **Створення значень для токенів**: Дотримуючись інструкцій провайдерів, інжектор створює значення для токенів.
3. **Зберігання мапінгу між токенами та їхніми значеннями**: Маючи токени провайдерів та їхні значення, Ditsmod формує відповідний мапінг.
4. **Автоматичне вирішення залежностей провайдерів**: Щоб створити значення для певного токена, треба спочатку створити відповідне значення для усіх залежностей його провайдерів. Тому інжектор вміє рекурсивно проходити по залежностям кожного провайдера, створювати граф залежностей, і потім послідовно створювати значення для кожної залежності в ланцюжку.
...
Давайте тепер спробуємо під час створення інжектора передати йому пустий масив. В такому разі виклик `injector.get()` кидатиме помилку:
...
Оскільки це дуже важливо, давайте ще раз опишемо схему роботи DI, але вже іншими словами. Коли створюється інжектор, він отримує провайдери, які для нього слугують інструкціями між токеном та значеннями, які треба видавати для цих токенів. Як бачите у попередньому прикладі коду, провайдери передаються до `Injector.resolveAndCreate()` і, як говорить назва цього методу, спочатку відбувається **вирішення** залежностей. А що таке "вирішення залежностей"? Це сканування кожного провайдера за допомогою [рефлектора][108], та визначення переліку токенів, від значень яких він залежить. А вже після того, як інжектор створено, рефлектор не використовується, тому `injector.get()` в наступному прикладі кидає помилку:
...
До речі, ще одна причина чому рефлектор не використовується під час виклику `injector.get()` полягає в тому, що токен провайдера може бути не лише класом, а й, наприклад, мати примітивний тип. Повний перелік типів токенів наступний:

1. Клас.
2. Будь-який об'єкт, окрім `null` та масиву. Рекомендуємо використовувати інстанс класу `InjectionToken<T>` у якості токена, оскільки він є дженеріком, який приймає параметр типу `T`. Цей параметр можна використовувати щоб асоціювати даний токен зі значенням, яке буде повертатись з реєстру інжектора.
3. `string`, `number` чи `symbol`.

Треба пам'ятати, що токени працюють в JavaScript-коді, а не у TypeScript-коді, тому у якості токенів не можуть виступати сутності, що оголошені з ключовими словами `interface`, `type`, `declare`, `enum` і т.д., оскільки після компіляції їх не буде у JavaScript-коді. Окрім цього, токени не можна імпортувати з ключовим словом `type`, оскільки у JavaScript-коді такого імпорту не буде.

До речі, в попередньому прикладі з кодом, коли ми передавали масив класів, інжектор їх сприймав також як провайдери. Тобто провайдери можуть бути у двох формах: або це клас, або це об'єкт з інструкціями для створення певного значення. Це означає, що обидва наступні інжектори отримують конфігурацію з еквівалентними інструкціями:
