...
До речі, ще одна причина чому рефлектор не використовується під час виклику `injector.get()` полягає в тому, що токен, який запитується в цьому методі, може бути не лише класом, а й, наприклад, мати примітивний тип.

### Коротка та довга форма передачі токенів у методах класу {#short-and-long-form-of-token-passing-in-class-methods}

Якщо у якості токена використовується клас, його також одночасно можна використовувати і як тип:
...
Тут мається на увазі, що `FirstService` є класом, і через це він може одночасно використовуватись і у якості TypeScript-типу, і у якості **токену**. Але що саме означає "використовується у якості токена"? - По-суті, таким оголошенням параметрів, яке ми зробили в конструкторі класу, ми говоримо DI: "Візьми токен `FirstService`, знайди в реєстрі провайдерів відповідне значення, і підстав його у якості значення даного параметра".

Нагадаємо, що токен - це ідентифікатор, який асоціюється із відповідною залежністю. Дуже важливо розуміти, що сам механізм використання токенів потрібний для JavaScript-runtime, тому у якості токенів не можна використовувати такі типи, які у TypeScript-коді ви оголошуєте з ключовими словами `interface`, `type`, `enum`, `declare` і т.п., бо їх не існує у JavaScript-коді. Окрім цього, токени не можна імпортувати з ключовим словом `type`, оскільки у JavaScript-коді такого імпорту не буде.
