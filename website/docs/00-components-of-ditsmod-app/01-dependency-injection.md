...
1. Під час створення інжектора, йому передається масив провайдерів - тобто масив інструкцій між тим, що у нього запитують (токеном), та тим, що він повинен видавати (значенням). В даному разі у якості провайдерів виступають класи `[Service1, Service2, Service3]`. Але де тут згадані "інструкції"? Справа в тому, що для створення інжектора, найкомпактніше передати йому провайдери у вигляді класів, які під капотом потім перетворяться на ось такі інструкції: `[{ token: Service1, useClass: Service1 }, { token: Service2, useClass: Service2 }, { token: Service3, useClass: Service3 }]`. Цей етап є дуже важливим для подальшого функціонування інжектора. Якщо ви не передасте усіх необхідних провайдерів, інжектор не матиме відповідних інструкцій, коли ви запитуєте певний токен.
2. Коли в інжектора запитують токен `Service3`, він проглядає конструктор цього класу, бачить залежність від `Service2`.
