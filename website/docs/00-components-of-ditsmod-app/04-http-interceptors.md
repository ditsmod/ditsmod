...
4. Якщо знайшовся обробник запиту, Ditsmod витягує [HttpFrontend][2] через DI (на рівні запиту), ставить його першим у черзі інтерсепторів і автоматично викликає. By default, цей інтерсептор відповідає за виклик ґардів та встановлення `req.pathParams` і `req.queryParams`.
