...
Як правило, розширення виконує свою роботу перед створенням обробників HTTP-запитів. Щоб змінити або розширити роботу застосунку, як правило, розширення використовує статичні метадані, закріплені за певними декораторами. З іншого боку, розширення може ще й динамічно додавати метадані такого самого типу, як і статичні метадані, зібрані з декораторів. Розширення можуть ініціалізуватись асинхронно, і можуть залежати один від одного.

По-суті, різні розширення працюють над багатовимірним масивом конфігураційних даних (метаданих). Цей масив відображає структуру застосунку:

1. він розбитий по модулям;
2. у кожному модулі є контролери або провайдери;
3. кожен контролер має один або більше роутів.

Як правило, кожне розширення вносить якісь правки над цим масивом, як на конвеєрі, і в кінцевому підсумку цей масив використовується фінальним розширенням, яке створює роути та встановлює відповідні обробники HTTP-запитів. Але не обов'язково щоб розширення працювали над конфігурацією та встановленням обробників HTTP-запитів; вони можуть ще й записувати логи чи збирати метрики для моніторингу, або виконувати будь-яку іншу роботу.
