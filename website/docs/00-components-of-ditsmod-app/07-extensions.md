...
Інший приклад. Модуль [@ditsmod/openapi][6] дозволяє створювати OpenAPI-документацію за допомогою власного декоратора `@oasRoute`. Без роботи розширень, Ditsmod буде ігнорувати цей новий декоратор.
...
3. починається по-модульна робота розширень:
    - для кожного модуля Ditsmod відбираються розширення, створені у цих модулях, або імпортовані в ці модулі;
    - метадані, зібрані у кожному з цих модулів, передаються до даних розширень і викликаються методи `init()` розширень.
...
Для спрощення, [Створення класу розширення][2] містить приклад, де вказано залежність `Extension2` від `Extension1`, але рекомендується указувати залежність саме від групи розширень, а не безпосередньо від конкретного розширенння. В такому разі, вам не потрібно знати імена усіх розширень, що входять у групу розширень, достатньо знати лише інтерфейс даних, що повертаються з `init()`.
