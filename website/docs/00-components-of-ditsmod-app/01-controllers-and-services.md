...
2. Контролер [одинак][8]. Його інстанс створюється по одному разу для кожного модуля під час ініціалізації застосунку.
...
Де `ctx.nodeRes` - це рідний Node.js об'єкт відповіді.
