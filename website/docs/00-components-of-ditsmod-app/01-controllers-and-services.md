...
Починаючи з v2.50.0, Ditsmod дає можливість працювати з контролером в двох режимах:

1. Контролер неодинак (по дефолту). Його інстанс створюється за кожним HTTP-запитом, і для нього доступний [інжектор][7] на рівні запиту.
2. Контролер [одинак][8]. Його інстанс створюється єдиний раз на рівні роуту під час ініціалізації застосунку; для нього [інжектор][7] на рівні запиту є недоступним; також для його методів (окрім конструктора) є недоступним використання [Dependecy Injection][7].

Перший варіант більш безпечний, коли потрібно працювати в контексті поточного запиту (клієнт надає певний ідентифікатор, який необхідно враховувати для формування відповіді). Другий варіант роботи помітно швидший і споживає менше пам'яті, але контекст запиту не можна зберігати у властивостях інстансу контролера, бо цей інстанс може одночасно використовуватись для інших клієнтів. В другому варіанті контекст запиту прийдеться передавати лише як аргумент для методів.
