...
2. Формування інжекторів з урахуванням декларацій імпортів/експортів кожного модуля.
...
Оголошення рівня провайдерів означає, що на цьому рівні інстанси зазначених провайдерів
...
Під "нижчим пріоритетом" мається на увазі, що якщо інструкції для DI протирічать одна-одній у
модулі і у контролері, DI буде використовувати інструкцію контролера.
...
Що означає "використовується як інтерфейс"? - Це означає, що якщо ви хочете підмінити `Logger`
своїм провайдером, ваш провайдер повинен мати такі ж імена методів, і таку ж сигнатуру цих методів як
вона є у `Logger`.
...
збільшиться на увесь застосунок, з урахуванням ієрархії інжекторів.
...
Інжектори - це інстанси класів, що мають масиви провайдерів та методи для створення інстансів цих провайдерів, з урахуванням всього ланцюжка їх залежностей.
...
_Уточнення:_ згадані чотири інжектори - це не загальна кількість інжекторів в застосунку, це
кількість інжекторів в ієрархії. Тобто, окремо взятий контролер працює саме із цими чотирма
інжекторами, але оскільки контролерів може бути багато, то й інжекторів відповідно може бути
більше.
...
дочірні інжектори на рівні роуту.
...
оголошених в масиві `providersPerReq`. І хоча цей інжектор інколи запитує інстанси провайдерів у батьківських інжекторів, але самостійно він їх не створює. Таким чином, в конструкторі контролера можуть бути одинаки з будь-якого рівня.
...
інстанси класу `Req` та `Res`, які в Ditsmod оголошені на рівні запиту:
...
про те, що він не може знайти провайдера для `Req` та `Res`. Але чому? Може їх треба
...
Причина криїться у невірно оголошеному рівні для `ErrorHandlerService`. Оскільки ви оголосили цей сервіс на рівні застосунку, його інстанс буде створювати інжектор на рівні застосунку. А це означає, що усі сервіси, що ви запитуєте в конструкторі, цей інжектор буде шукати тільки в масиві, що ви передали у
`providersPerApp`.

Разом з тим, `Req` та `Res` в Ditsmod оголошені на рівні запиту, тобто ці сервіси
...
2. оголосіть `ErrorHandlerService` на рівні запиту. Щоправда, в такому разі видимість
`ErrorHandlerService` все ще буде обмежуватись тим модулем, де ви оголосили цей провайдер. Як
правильно оголосити обробника помилок для контролера, прогляньте [репозиторій ditsmod seed][14].

### Поточний інжектор

Безпосередньо сам інжектор вам рідко може знадобиться, але ви його можете отримати
