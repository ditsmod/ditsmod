...
Проглянути код з прикладами використання `@ditsmod/i18n` можна у [репозиторію Ditsmod][2], хоча зручніше його проглядати локально, тому краще спочатку клонуйте його:
...
У прикладі `examples/15-i18n` є два модуля, кожен із яких має приблизно таку структуру директорій з файлами для перекладу: 
...
Це рекомендована структура директорій. Як бачите, кожен модуль має переклад у теці `locales`, яка містить дві теки:
...
Кожен базовий словник повинен імплементувати інтерфейс `Dictionary`, який має єдину вимогу - щоб у словнику був метод `getLng()`, що повертає скорочення назви мови по стандарту [ISO 639][3] (наприклад, скорочення для англійської та української мов - en, uk).
...
Як бачите, в масиві передаються групи словників, де на першому місці завжди повинен йти клас з базовим словником. В даному разі передаються дві групи словників з базовими класами `CommonDict` та `ErrorDict`. Не дозволяється змішувати словники з різних груп. Якщо ви змішаєте словники з різних груп, TypeScript не зможе вам про це сказати, тому рекомендується використовувати функцію `getDictGroup()` для кращого контролю типів класів:
...
[2]: https://github.com/ditsmod/ditsmod
[3]: https://uk.wikipedia.org/wiki/ISO_639
