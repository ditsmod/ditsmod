...
## Експорт, імпорт, долучення {#export-import-append}
...
В даному випадку, `Service1` буде додаватись до усіх модулів застосунку на рівні HTTP-запиту. Як бачите, експортувати можна також і цілі модулі. В даному разі, усі провайдери, що експортуються з `Module1`, також будуть додаватись до кожного модуля застосунку на відповідному рівні.
...
Якщо вам не потрібно імпортувати провайдери та [розширення][2] в поточний модуль, а потрібно всього лиш долучати зовнішній модуль до path-префікса поточного модуля, можна скористатись масивом `appends`:
...
## Колізії провайдерів {#provider-collisions}

Колізії провайдерів виникають тоді, коли у поточний модуль імпортуються різні провайдери, що мають один і той самий токен.
