...
Як бачите, в конструкторі `Service2` вказано статичний тип даних для параметра `service1`. Це типовий приклад для застосунків, коли один клас залежить від іншого, і ця залежність вказується в конструкторі класу. Чому це називається залежністю? - Тому що перед тим, як створити інстанс `Service2`, треба спочатку створити інстанс `Service1`.
...
Тобто інформація про тип параметра в конструкторі `Service2` втрачена. Це нам не підходить, оскільки в такому разі ми не зможемо автоматично (програмно) дізнатись, що `Service2` залежить від `Service1`. Але якщо ми використаємо декоратор класу, TypeScript-компілятор вивантажить більше JavaScript-коду з інформацією про статичну типізацію. Наприклад, давайте скористаємось декоратором `injectable`:
...
// Зберігаємо метадані, що асоціюються з Service1
Reflect.defineMetadata('any-metadata-key', 'some-value', Service1);

// Зчитуємо раніше збережені метадані (повертає "some-value")
Reflect.getMetadata('any-metadata-key', Service1);
```

Як бачите, розширене API у `Reflect` дозволяє закріпляти за певним класом метадані навіть без декораторів. На початку цього скрипта є `import 'reflect-metadata/lite'`, за допомогою якого ця бібліотека робить так званий "monkey patching", тобто динамічно додає API до стандартного класу `Reflect`. Якщо ви цей імпорт закоментуєте, то код перестане працювати, і ви отримаєте помилку, яка говоритиме, що `Reflect` не має методу `defineMetadata()`. Це саме стосується декораторів - без `reflect-metadata` вони перестануть працювати. Це треба пам'ятати під час тестування, коли у файлі тесту ви не імпортуєте `@ditsmod/core` (в якому автоматично імпортується `reflect-metadata/lite`). Наприклад, у Jest можна встановити опцію в конфігураційному файлі `setupFilesAfterEnv: ['reflect-metadata/lite']`, щоб не робити імпорт цієї бібліотеки вручну.

Тепер давайте поглянемо, які високорівневі інструменти має Ditsmod для роботи з рефлектором. Ускладнимо попередній приклад, щоб побачити як можна витягувати метадані та формувати складні ланцюжки залежностей. Розглянемо три класи з наступною залежністю `Service3` -> `Service2` -> `Service1`. Вставте у `src/app/services.ts` наступний код:
