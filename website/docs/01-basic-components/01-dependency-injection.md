...
- **Інжектор** - цей термін у побуті використовується рідко, але сама його концепція у побуті досить поширена і трохи нагадує "Маркетплейс" із сервісом доставки (як Amazon у США). Спочатку провайдери передають свої товари чи сервіси у Маркетплейс, а потім споживачі звертаються до цих Маркетплейсів з токенами, щоб отримати відповідний товар чи сервіс. В технічному плані, спочатку розробники реєструють провайдери в інжекторах, а потім використовуючи токени отримують відповідні значення певного провайдера.
...
Ланцюжок залежностей провайдерів може бути досить складним, а ієрархія інжекторів ще додає складності. Давайте розглянемо простий випадок, а потім ускладнемо його. Отже в наступному прикладі `Service` залежить від `Config`, причому обидва провайдери для цих сервісів передаються в один інжектор:
...
1. спочатку він прогляне масив своїх провайдерів і знайде там `Service`, тому він "знатиме", що коли у нього запитують `Service`, треба видавати інстанс цього класу;
2. потім він прогляне список залежностей у `Service` і знайде там `Config`;
3. потім він прогляне масив своїх провайдерів і знайде там `{ token: Config, useValue: { one: 1, two: 2 } }`, тому він "знатиме", що коли у нього запитують `Config`, він повинен видати `{ one: 1, two: 2 }`;
4. потім він прогляне список залежностей у провайдера з токеном `Config` і не знайде там залежностей;
5. тому для створення інстансу `Service` буде використовуватись значення `{ one: 1, two: 2 }`. Зверніть увагу, що під час створення інстансу `Service` не буде створюватись інстанс `Config`, а буде використано готовий об'єкт `{ one: 1, two: 2 }`, бо такі інструкції передались у масиві провайдерів під час створення батьківського інжектора.
...
Тепер ускладнемо приклад: передамо один провайдер на один рівень ієрархії, а його залежність - на інший. В такому разі залежність завжди потрібно передавати на вищий рівень, бо пошук залежності завжди відбувається знизу-доверху по ієрархії, і ніколи - зверху-донизу.

В наступному прикладі, `Service` залежить від `Config`, тому `Config` передається у батьківський інжектор, а `Service` - у дочірній інжектор. В такому разі пошук знизу-доверху (`Service -> Config`) в ієрархії спрацює:
...
Тепер давайте порушимо правило, яке говорить, що залежність може бути на тому самому рівні ієрархії, що і провайдер, або на вищому рівні. Отже, `Service` у нас буде на вищому рівні (в батьківському інжекторі), а `Config` - на нижньому рівні (в дочірньому інжекторі):
