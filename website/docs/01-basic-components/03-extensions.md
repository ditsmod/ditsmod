...
Під час виконання `stage1()` будь-яке розширення може [динамічно додавати провайдери][7] на будь-який [рівень ієрархії][8]. Лише після завершення `stage1()` в усіх розширеннях всіх модулів, створюються фінальні інжектори - один на рівні застосунку, та по одному на рівні кожного модуля. Інжектор модуля передається аргументом у `stage2(injectorPerMod)`. На цьому етапі також можна додавати провайдери, але лише на рівнях, нижчих за модуль. Це мають робити ті розширення, для яких ці провайдери призначені, і вони ж самостійно створюють відповідні фінальні інжектори.
...
Це найпростіший спосіб реєстрації розширення, який підходить лише для випадків, коли вам достатньо щоб розширення працювало у тому модулі, де воно оголошено і зареєстровано. Для більш складних конфігурацій, ви можете передати об'єкт, що має наступний тип:
...
Зверніть увагу, що `extensionGroupMeta.groupData` завжди буде мати масив результатів, незалежно від того, чи в поточному модулі `Extension1` входить у групу розширень, чи ні. Тут `extensionGroupMeta` має наступний інтерфейс:
...
Тепер, коли ви ознайомились з `ExtensionManager`, важливо наголосити на тому, що пошук груп розширень відбувається за токенами — за тими класами розширень, які ми раніше вказали у властивості `groups`:
...
export class BodyParserExtension implements Extension<void> {
  ...
  async stage1() {
    ...
    extensionGroupMeta.groupData.forEach((routeExtensionMeta) => {
      ...
      controllersMeta.forEach(({ providersPerRou, providersPerReq, httpMethods, scope }) => {
...
В даному разі, у метадані контролера додається HTTP-інтерсептор в масив `providersPerReq` або `providersPerRou` (в залежності від режиму роботи контролера). Зверніть увагу, що тут створюється [ієрархія інжекторів][8], яка використовується лише щоб отримати значення для токена `BodyParserConfig`, що вказує нам чи потрібно додавати інтерсептор. Після цього дані інжектори нікуди більше не передаються, тобто видаляються з пам'яті.
...
### Передача готових ресурсів та стейту (ValueProvider) {#transferring-prepared-resources-and-state}

Якщо розширення асинхронно ініціалізує певний зовнішній ресурс (наприклад, підключення до бази даних, клієнт Redis або зконфігурований SDK), цей ресурс можна передати у довгоживучий інжектор застосунку за допомогою `ValueProvider`.
...
Після цього будь-який сервіс чи контролер у застосунку може інжектувати `DbClient` через стандартний механізм Dependency Injection.
