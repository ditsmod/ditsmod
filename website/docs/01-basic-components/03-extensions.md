...
Як бачите, спочатку оголошено контролер та інтерсептор, які символізують тут розрізнені метадані. Потім, у виділених рядках, показано функцію `extension1()`, яка тут умовно символізує функціональність розширення. Тобто, по-суті, розширення збирає до купи усі метадані, передає їх до DI реєстру, створює інжектор, і в кінцевому підсунку створює обробника HTTP-запитів.
...
Як бачите, розширення можуть вказувати залежність від сервісів, провайдери яких можуть бути оголошені на рівні модуля чи застосунку. Але майте на увазі, що цей інжектор формується ще до того, як розширення починають працювати, тому до нього не передаються провайдери від інших розширень.
