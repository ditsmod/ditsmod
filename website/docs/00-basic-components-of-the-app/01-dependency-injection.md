...
4. У попередньому пункті, по-суті, у нього запитують токен `Service2`, тому він проглядає масив провайдерів і бачить там інструкцію `{ token: Service2, useClass: Service2 }`, тому він "розуміє", що для токена `Service2` треба видавати інстанс класу `Service2`.
5. Потім проглядає конструктор у `Service2`, бачить залежність від `Service1`.
6. У попередньому пункті у нього запитують токен `Service1`, тому він проглядає масив провайдерів і бачить там інструкцію `{ token: Service1, useClass: Service1 }`, тому він "розуміє", що для токена `Service1` треба видавати інстанс класу `Service1`.
