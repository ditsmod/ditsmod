...
- `normalize(normalizedModuleMeta)`: Валідує та нормалізує опції декоратора, повертаючи структурований об'єкт метаданих, який зберігається у `normalizedModuleMeta.initMeta`.
...
1. Кастомні параметри (такі як `path` або `guards`) автоматично додаються в Map `dynamicModule.initOptions` під токен-ключем декоратора.
